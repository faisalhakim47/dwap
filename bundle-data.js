...
(async function () {
  ...
  let maxVillageId = 0;
  let maxNameLength = 0;
  let minProvinceId = Number.MAX_VALUE;
  let minRegencyId = Number.MAX_VALUE;
  let minDistrictId = Number.MAX_VALUE;
  let minVillageId = Number.MAX_VALUE;
  ...
  for (const village of villages) {
    ...
    if (provinceId < minProvinceId) {
      minProvinceId = provinceId;
    }

    if (regencyId < minRegencyId) {
      minRegencyId = regencyId;
    }

    if (districtId < minDistrictId) {
      minDistrictId = districtId;
    }

    if (villageId < minVillageId) {
      minVillageId = villageId;
    }
  }
